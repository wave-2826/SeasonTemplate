 ...
 * 
 * This class wraps {@link Odometry} to fuse latency-compensated vision measurements with encoder measurements. Robot
 * code should not use this directly- Instead, use the particular type for your drivetrain (e.g.,
 * {@link DifferentialDrivePoseEstimator}). It is intended to be a drop-in replacement for {@link Odometry}; in fact, if
 * you never call {@link OdometryPoseEstimator#addVisionMeasurement} and only call {@link OdometryPoseEstimator#update}
 ...
 *
 * <p>
 * {@link OdometryPoseEstimator#addVisionMeasurement} can be called as infrequently as you want; if you never call it
...
public class OdometryPoseEstimator<T> {
     ...
     *            global measurements from vision less. This matrix is in the form [x, y, theta]ᵀ, with units in meters
     *            and radians.
     */
     ...
     *
     * <p>
     * The gyroscope angle does not need to be reset here on the user's robot code. The library automatically takes care
    ...
    public Optional<Pose2d> sampleAt(double timestampSeconds) {
        ...
        if(m_odometryPoseBuffer.getInternalBuffer().isEmpty()) { return Optional.empty(); }
    ...
    private void cleanUpVisionUpdates() {
        ...
        if(m_visionUpdates.isEmpty() || oldestOdometryTimestamp < m_visionUpdates.firstKey()) { return; }
     ...
     *            time source by calling {@link OdometryPoseEstimator#updateWithTime(double,Rotation2d,Object)} then you
     *            must use a timestamp with an epoch since FPGA startup (i.e., the epoch of this timestamp is the same
     *            epoch as {@link edu.wpi.first.wpilibj.Timer#getFPGATimestamp()}.) This means that you should use
     *            {@link edu.wpi.first.wpilibj.Timer#getFPGATimestamp()} as your time source or sync the epochs.
     */
     ...
     * measurements until a subsequent call to {@link OdometryPoseEstimator#setVisionMeasurementStdDevs(Matrix)} or this
     * method.
     *
     * @param visionRobotPoseMeters The pose of the robot as measured by the vision camera.
     * @param timestampSeconds The timestamp of the vision measurement in seconds. Note that if you don't use your own
     *            time source by calling {@link #updateWithTime}, then you must use a timestamp with an epoch since FPGA
    ...
    /**
     * Represents a vision update record. The record contains the vision-compensated pose estimate as well as the
    ...
    private static final class VisionUpdate {
        ...
        /**
         * Returns the vision-compensated version of the pose. Specifically, changes the pose from being relative to
